...
from dps.spark.prep.romance_prep import (
    spanish_word_ratio_filter,
    portuguese_word_ratio_filter,
    french_word_ratio_filter,
    italian_word_ratio_filter,
    romanian_word_ratio_filter,
    replace_romance_pii,
    remove_html_tags,
    bad_words_filter,
    reduce_emoticon,
...
def romance_job(config_path):
    ...
    with session_fn("romance text processing job") as spark:
        ...
        proc_rdd: RDD = (
            ...
            .filter(
                lambda x: spanish_word_ratio_filter(
                    x["text"],
                    conf["spanish_word_ratio_filter"],
                )
            )
            .filter(
                lambda x: french_word_ratio_filter(
                    x["text"],
                    conf["french_word_ratio_filter"],
                )
            )
            .filter(
                lambda x: portuguese_word_ratio_filter(
                    x["text"],
                    conf["portuguese_word_ratio_filter"],
                )
            )
            .filter(
                lambda x: romanian_word_ratio_filter(
                    x["text"],
                    conf["romanian_word_ratio_filter"],
                )
            )
            .filter(
                lambda x: italian_word_ratio_filter(
                    x["text"],
                    conf["italian_word_ratio_filter"],
                )
            )
            .map(
                lambda x: dict(

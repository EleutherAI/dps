...
from dps.spark.prep.chinese_prep import (
    chinese_word_ratio_filter,
    chinese_bad_words_filter,
    # reduce_chinese_emotion,
    remove_html_tags,
)
...
def preprocess_text(text: str):
    processing_functions = [
        ...
        remove_whitespace,
        replace_email_and_url,
        # reduce_chinese_emoticon
        # chinese_remove_repeated_text,
    ]
...
def chinese_job(config_path: str):
    ...
    with session_fn("chinese text processing job") as spark:
        sc: SparkContext = spark.sparkContext
        proc_rdd: RDD = (
            sc.textFile(input_paths)
            .repartition(conf["n_dist"])
            .flatMap(read_line)
            .filter(lambda x: chinese_bad_words_filter(x["text"]))
            .filter(
                lambda x: doc_len_filter(
                    x["text"], conf["min_doc_len"], conf["max_doc_len"]
                )
            )
            # .filter(lambda x: chinese_mean_word_len_filter(x["text"], conf["min_mean_word_len"], conf["max_mean_word_len"]))
            # .filter(lambda x: chinese_symbol_to_word_ratio_filter(x["text"], conf["symbol_to_word_ratio"]))
            .filter(
                lambda x: bullet_ellipsis_filter(
                    x["text"], conf["bullet_point_ratio"], conf["ellipsis_ratio"]
                )
            )
            .filter(
                lambda x: chinese_word_ratio_filter(
                    x["text"], conf["japanese_word_ratio"]
                )
            )
            ...
            # .filter(lambda x: chinese_frequent_char_existence_filter(x["text"], conf["freq_char_cnt"]))
            # .filter(lambda x: reduce_chinese_emoticon(x["text"]))

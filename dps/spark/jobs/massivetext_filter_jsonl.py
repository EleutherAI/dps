...
from ..utils.massivetext_filter import (
    doc_len_filter,
...
def massivetext_filter_jsonl(
    ...
    min_doc_len: int = 50,
    max_doc_len: int = 100000,
    min_mean_word_len: int = 3,
    max_mean_word_len: int = 10,
    symbol_to_word_ratio: float = 0.1,
    bullet_point_ratio: float = 0.9,
    ellipsis_ratio: float = 0.3,
    korean_word_ratio: float = 0.5,
):
        ...
        proc_rdd: RDD = (
            ...
            .filter(
                lambda x: mean_word_len_filter(
                    x["text"], min_mean_word_len, max_mean_word_len
                )
            )
            .filter(
                lambda x: symbol_to_word_ratio_filter(x["text"], symbol_to_word_ratio)
            )
            .filter(
                lambda x: bullet_ellipsis_filter(
                    x["text"], bullet_point_ratio, ellipsis_ratio
                )
            )
            .filter(lambda x: korean_word_ratio_filter(x["text"], korean_word_ratio))
        )
        proc_rdd.repartition(n_output).flatMap(to_json).saveAsTextFile(output_dir)
